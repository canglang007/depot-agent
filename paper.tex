...
\centering
\small
\begin{tabular}{@{}p{2.0cm} p{4.0cm} p{5.0cm}@{}}
\toprule
成员 & 负责章节 & 主要贡献 \\
\midrule
史瑞恺 & 第1章（引言）、第2章（相关工作）、第4.2节（AST提取器） & 选题背景调研、Gap分析与问题定义、相关工作与对比分析 \\
\hline
韩可可 & 第4.3-4.6节（解析器/安装器/执行器/缓存）、第6章（工程化实现与开源） & 系统设计、实验方案设计、工程化实现方案 \\
\hline
刘卓明 & 第3章（问题定义）、第4.1/4.7/4.8节（架构/创新/反馈）、第5章（实验）、第7-8章（讨论/结论） & 论文框架设计、实验数据分析与撰写、论文统稿 \\
\bottomrule
\end{tabular}
\end{table}
